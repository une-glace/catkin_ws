...
serial::Serial serialPort(port, baudrate, serial::Timeout::simpleTimeout(1000));
	...
	static std::time_t now = std::time(nullptr);
	char buffer[80];
	pose1.pose.position.x = 100; pose1.pose.position.y = 200; pose1.pose.position.z = 20;
	pose2.pose.position.x = -20; pose2.pose.position.y = -50; pose2.pose.position.z = 20;
	pose3.pose.position.x = 33; pose3.pose.position.y = 120; pose3.pose.position.z = 20;
	pose.pose.position.x = 50; pose.pose.position.y = 50; pose.pose.position.z = 20;
	...
	ros::Subscriber plane_local_v_sub = nh.subscribe<geometry_msgs::TwistStamped>("/plane_0/mavros/local_position/velocity_local", 10, local_v_sb);
	ros::Subscriber plane_local_v_sub_body = nh.subscribe<geometry_msgs::TwistStamped>("/plane_0/mavros/local_position/velocity_body", 10, local_v_body_sb);
	ros::Subscriber plane_gpsvel = nh.subscribe<geometry_msgs::TwistStamped>("/plane_0/mavros/global_position/raw/gps_vel", 10, gpsvel_sb);
	ros::Subscriber plane_local_pos_odom = nh.subscribe<nav_msgs::Odometry>("/plane_0/mavros/local_position/odom", 10, local_pos_odom_sb);
	ros::Subscriber plane_global_pos_odom = nh.subscribe<nav_msgs::Odometry>("/plane_0/mavros/global_position/local", 10, global_pos_odom_sb);
    ros::Subscriber plane_gpsraw = nh.subscribe<mavros_msgs::GPSRAW>("/plane_0/mavros/gpsstatus/gps1/raw", 10, gpsraw_sb);
	ros::Subscriber plane_mav_alt = nh.subscribe<mavros_msgs::Altitude>("/plane_0/mavros/altitude", 10, mav_alt_sb);
	ros::Subscriber plane_rel_alt = nh.subscribe<std_msgs::Float64>("/plane_0/mavros/global_position/rel_alt", 10, rel_alt_sb);
	ros::Subscriber plane_hud = nh.subscribe<mavros_msgs::VFR_HUD>("/plane_0/mavros_msgs/VFR_HUD", 10, hud_sb);
	ros::Publisher target_pos_pub = nh.advertise<geometry_msgs::PoseStamped>("/plane_0/mavros/setpoint_position/local", 10);
	ros::Publisher target_vel_pub = nh.advertise<geometry_msgs::Twist>("/plane_0/mavros/setpoint_velocity/cmd_vel_unstamped", 10);
	...
	// serialPort.write("59\n")
			...
			if(local_pos.pose.position.z>40){
				ROS_INFO("Ready to choose!!!");
			}
			
			if (local_pos.pose.position.z>40 && std::cin >> target)
