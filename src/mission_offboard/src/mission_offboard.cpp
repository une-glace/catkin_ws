...
//	int delt_z = global_pos_odom.pose.pose.position.z - local_pos.pose.position.z;
    ...
    while (ros::ok()) {
//	delt_z = global_pos_odom.pose.pose.position.z - local_pos.pose.position.z;
	delt_pose.pose.position.x = local_pos.pose.position.x - global_pos_odom.pose.pose.position.x;
	delt_pose.pose.position.y = local_pos.pose.position.y - global_pos_odom.pose.pose.position.y;
	delt_pose.pose.position.z = local_pos.pose.position.z - global_pos_odom.pose.pose.position.z;
	...
	// serialPort.write("59\n")
		...
		//	if (global_pos_odom.pose.pose.position.z > 42)
			 if  (wp.wp_seq >= 7)
              {	offb_set_mode.request.custom_mode = "OFFBOARD";mission_status=0;

				if(set_mode_client.call(offb_set_mode)&&offb_set_mode.response.mode_sent){
             //      pose.pose.position.z += delt_z;
			...
			case 1:
			    ...
			    // if(distance>15){
					...
					// ROS_ERROR(&timep2);
					// ROS_FATAL(&timep3);
						//printf("not move :timep222222222222:%d\n",timep2);
						//printf("move:timep333333:%d\n",timep3);
...
// 	static std::time_t now = std::time(nullptr);
// 	char buffer[80];
// 	pose1.pose.position.x = 15.6; pose1.pose.position.y = 19.3; pose1.pose.position.z = 15;
// 	pose2.pose.position.x = -20; pose2.pose.position.y = -50; pose2.pose.position.z = 15;
// 	pose3.pose.position.x = 33; pose3.pose.position.y = 120; pose3.pose.position.z = 15;
// 	pose.pose.position.x = 50; pose.pose.position.y = 50; pose.pose.position.z = 15;
...
// 			if (local_pos.pose.position.z > 32 )
...
// 					// ROS_ERROR(&timep2);
// 					// ROS_FATAL(&timep3);
// 						//printf("not move :timemainp222222:%d\n",timep2);
// 						//printf("move:timep333333:%d\n",timep3);
...
std::ofstream data(filename);
	...
	geometry_msgs::PoseStamped pose, delt_pose, target, ori_pose;
	mavros_msgs::Thrust thrust;
	pose.pose.position.x =-50;
		...
		//	if (global_pos_odom.pose.pose.position.z > 42)
			 if  (wp.wp_seq >= 7)
              {	offb_set_mode.request.custom_mode = "OFFBOARD";mission_status=0;
				...
				if(set_mode_client.call(offb_set_mode)&&offb_set_mode.response.mode_sent){
             //      pose.pose.position.z += delt_z;
			 		target.pose.position.x = pose.pose.position.x - ori_pose.pose.position.x;
					target.pose.position.y = pose.pose.position.y - ori_pose.pose.position.y;
					target.pose.position.z = pose.pose.position.z - ori_pose.pose.position.z;
				  target_pos_pub.publish(target);
				  ROS_INFO("offboard enabled");
				  mission_mode = offboard;
				  }
